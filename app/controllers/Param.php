...
{
    public function getContentFromApi($url, $token, $query = "")
    {
    ...
    function cloudflare()
    {
        $zoneIdentifier = "8be47c6e79fd1e70c9692e1b7bbcc2c9";
        $link = "https://api.cloudflare.com/client/v4/zones/$zoneIdentifier/dns_records?per_page=900";

        $returnedJson = json_decode($this->getContentFromApi($link, "13d43cde15effa32181fbd63d341eb581da26"));

        echo "<pre>";

        $pagination = null;

        $currentPage = $returnedJson->result_info->page;
        $maxPage = $returnedJson->result_info->total_pages;
        $count = 0;
        ...
        #while ($currentPage < $maxPage) {
            if($pagination == null)
                $pagination = $returnedJson->result_info;
        
            foreach ($returnedJson->result as $key => $value) {
                ...
                $result = $this->getContentFromApi("https://api.cloudflare.com/client/v4/zones/$zoneIdentifier/dns_records/$value->id", 
                    "13d43cde15effa32181fbd63d341eb581da26");
